...
class App:
    ...
    def _load_session(self, path: str):
        ...
        self.end2end    = meta.get("end2end", "False").lower() == "true"

    # ── parse YOLOv8/v11 raw output ─────────────────────────────
    @staticmethod
    def _parse_yolo(raw: np.ndarray, orig_w: int, orig_h: int,
                    class_names: list, conf_thr: float = 0.25,
                    iou_thr: float = 0.45, end2end: bool = False):
        """
        รองรับ 2 format:

        end2end=True  (simplify+NMS built-in)
          shape: [1, N, 6]  → each row = [x1,y1,x2,y2, conf, cls_id]
          bbox unit: pixel ใน imgsz (320x320) → ต้อง scale กลับ

        end2end=False (raw anchors, ต้องทำ NMS เอง)
          shape: [1, 4+nc, A] → transpose → [A, 4+nc]
          bbox unit: pixel ใน imgsz (320x320) → ต้อง scale กลับ
        """
        INF = 320   # imgsz ที่ใช้ train/export
        sx  = orig_w / INF
        sy  = orig_h / INF
        ...
        if end2end:
            # ── Format 1: [1, N, 6]  x1 y1 x2 y2 conf cls ───────────
            preds = raw[0]                          # [N, 6]
            for row in preds:
                x1, y1, x2, y2, conf, cid = row
                cid = int(cid)
                if conf < conf_thr or cid >= len(class_names):
                    continue
                results.append((
                    class_names[cid], cid, float(conf),
                    int(x1*sx), int(y1*sy),
                    int(x2*sx), int(y2*sy)
                ))

        else:
            # ── Format 2: [1, 4+nc, A] raw anchors ───────────────────
            preds     = raw[0].T                    # [A, 4+nc]
            boxes_raw = preds[:, :4]                # cx,cy,w,h (pixel in 320)
            scores    = preds[:, 4:]                # [A, nc]
            ...
            # cx,cy,w,h (pixel in 320) → x1,y1,x2,y2 (pixel in orig)
            ...
            for cid in np.unique(class_ids):
                ...
                for k in keep:
                    i = idx[k]
                    results.append((
                        class_names[cid], int(cid), float(confs[i]),
                        int(x1[i]), int(y1[i]),
                        int(x2[i]), int(y2[i])
                    ))
    ...
    def _loop(self):
        ...
        while self.running:
            ...
            # (_parse_yolo scale จาก 320→orig แล้ว ต้อง scale ต่อถึง display)
            dsx, dsy = CAM_W / orig_w, CAM_H / orig_h
            detections = [
                (cls, cid, conf,
            ...
            # นับจำนวน bbox ต่อ class โดยตรง
            ...
            t1 = time.time()
            frame_count  += 1
            fps_acc       += 1.0 / (t1 - t0 + 1e-9)
            if (t1 - self._fps_last_upd) >= FPS_UPDATE_SEC:
                self._fps_display  = fps_acc / frame_count
                fps_acc, frame_count = 0.0, 0
                self._fps_last_upd = t1
